...
function sftp_download(edy, sftp, obj, __callback) {
	...
	async.map(obj.list, function(file, callback) {
		...
		var real_file = file.one ? path.join(local_dir, file.filename) : path.join(local_dir, file.root.replace(root.replace("./", ""), ''), file.filename);
		var ftp_file = file.one ? path.join(file.root, file.filename) : path.join(root, file.root.replace(root.replace("./", ""), ''), file.filename)
...
function sftp_is_dir(sftp, ftp_file, cb) {
  
  sftp.stat(ftp_file, function(err, stat) {

    if (err) {
      cb(false, stat);
    } else {
      cb(stat.isDirectory(), stat);  
    }
...
function sftp_tree(edy, callback) {
	...
	sftp_connect(edy, function(sftp){
	  ...
	  sftp_is_dir(sftp, root, function(isDir, stat){
	    if (isDir) {
	      sftp_traverse(sftp, root, obj);    
	    } else {
	      sftp_traverse_file(sftp, root, obj);
	    }
	  })
...
function sftp_traverse_file(sftp, root, obj) {
  ...
  sftp.stat(root, function(err2, stat){
